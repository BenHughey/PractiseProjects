...
var GameState = {
  ...
  create: function() {      
	...
	//uses our placeItem method to place selected item on screen 
    this.background.events.onInputDown.add(this.placeItem, this);

    this.pet = this.game.add.sprite(100, 400, 'pet');
	//sets anchor point to middle
    this.pet.anchor.setTo(0.5);

    //spritesheet animation		animation key, frame sequence, framerate, looping y/n?
	...
	//apple has a health value of 20
    this.apple.customParams = {health: 20};
	//when apple is clicked, pickItem method is initiated
    ...
    this.game.add.text(140, 20, 'Fun:', style);
										//(coordinates, empty string, styling attribues)
    this.healthText = this.game.add.text(80, 20, '', style);
    this.funText = this.game.add.text(185, 20, '', style);

	//refreshes stats using our refreshStats method
    this.refreshStats();
  ...
  pickItem: function(sprite, event) {
    ...
    if(!this.uiBlocked) {
      ...
      //alpha to indicate selection, makes sprite semi transparent
      sprite.alpha = 0.4;
  ...
  placeItem: function(sprite, event) {
	//checks if an item is selected and user input isn't blocked
    if(this.selectedItem && !this.uiBlocked) {
		//sets coordinates of click/ touch event
      var x = event.position.x;
      var y = event.position.y;

	  //places a copy of selected item in location of touch event
      var newItem = this.game.add.sprite(x, y, this.selectedItem.key);
      newItem.anchor.setTo(0.5);
	  //gives it the health/fun values of original
      newItem.customParams = this.selectedItem.customParams;

	  //blocks user input
      this.uiBlocked = true;

      //tween animation to move the pet towards the item
      var petMovement = this.game.add.tween(this.pet);
					//(coordinates, miliseconds)
